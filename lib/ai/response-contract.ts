...
const structuredResponseSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.mode === 'text' && value.ui !== null) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'mode=text cannot include a ui payload',
        path: ['ui'],
      });
    }
  });
...
export class StructuredResponseContractError extends Error {
  constructor() {
    super('The server returned an invalid response. Please try again.');
    this.name = 'StructuredResponseContractError';
  }
}

/**
 * Decode an untrusted response at a process boundary.
 *
 * Callers must not inspect intent or commit state before this succeeds.
 */
export function decodeStructuredResponse(payload: unknown): StructuredResponse {
  const structured = validateStructuredResponse(payload);
  if (!structured) throw new StructuredResponseContractError();
  return structured;
}
